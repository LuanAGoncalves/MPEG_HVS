...
class Encoder:
    def __init__(self, videoName, quality = 50, sspace = 15, mode = '420', search = 0, hvsqm = 0, flat = 10.0, p = 2.0):
        ...
        if self.hvsqm != 1:
            self.Z_inter = self.genQntb(self.quality)
            self.Z_inter[:,:,0] = 16.0*np.ones((8,8), float)
        self.numBits = 0
        self.kbps =  0
        if self.hvsqm == 1:
            self.genHVStables()
        ...
        for f in range(0,len(self.video)):
            ...
            # B-frame
                ...
                if self.hvsqm == 1:
                    ...
                    encoder = jpeghvs.Encoder(bframe.bframe, self.quality, self.hufftables, ZtabVec, self.mode)
                    self.avgBits.append(encoder.avgBits)
                    self.CRate.append(encoder.CRate)
                    self.redundancy.append(1.-(1./encoder.CRate))
                    self.numBits += encoder.NumBits
                else:
                    encoder = jpeg.Encoder(bframe.bframe, self.quality, self.hufftables, self.Z_inter, self.mode)
                    self.avgBits.append(encoder.avgBits)
                    self.CRate.append(encoder.CRate)
                    self.redundancy.append(1.-(1./encoder.CRate))
                    self.numBits += encoder.NumBits
            ...
            # P-frame
                ...
                if self.hvsqm == 1:
                    ZtabVec = [self.hvstables, MV, self.sspace]
                    encoder = jpeghvs.Encoder(pframe.pframe, self.quality, self.hufftables, ZtabVec, self.mode)
                    self.avgBits.append(encoder.avgBits)
                    self.CRate.append(encoder.CRate)
                    self.redundancy.append(1.-(1./encoder.CRate))
                    self.numBits += encoder.NumBits
                else:
                    encoder = jpeg.Encoder(pframe.pframe, self.quality, self.hufftables, self.Z_inter, self.mode)
                    self.avgBits.append(encoder.avgBits)
                    self.CRate.append(encoder.CRate)
                    self.redundancy.append(1.-(1./encoder.CRate))
                    self.numBits += encoder.NumBits
...
#                    motionVec.append(tuple(np.array(vecSTR[i].split(','), int)))
                ...
                if self.hvsqm == 1:
                    ...
                    sequence.append([countfr, jpeg.Decoder(ch, self.hufftables, self.Z_inter, [self.shape, self.quality, self.mode])._run_(), motionVec])
                count += 1
                countfr += 1
