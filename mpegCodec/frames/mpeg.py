        ...
        if self.search == 0:    # Full search.
            for i in range(0, rR, self.mbr):
                for j in range (0, rC, self.mbc):
                    mad = (1.0/float(self.mbr*self.mbc))*(np.sum(cv2.absdiff(self.currentfr[i:i+self.mbr,j:j+self.mbc, 0], pastfr[self.sspace+i:self.sspace+i+self.mbr, self.sspace+j:self.sspace+j+self.mbc,0])))
                    x = 0
                    y = 0
